    ...
    public void act()
    {
        ...
        if(!getWorld().getObjects(Enemy.class).isEmpty()){
            if(counter >= speed){
                ...
                for(int i = 0; i < 10 + Greenfoot.getRandomNumber(20-10); i++) getWorld().addObject(new NormalBullet(actualX, actualY, 30, 30, Math.random() * 500.0 + (actualX - 250.0), Math.random() * 500.0 + (actualY - 250.0), 6, 1.0, 1.0, false, "bullet"), getGlobalX(), getGlobalY());
            }
            counter++;
            numOfRings--;
            if(numOfRings <= 0){
